    ...
    <div id="wrapper">
      ...
      <div id="filterBar">
        <select v-model="filterMall" id="filterMall">
          ...
          <option v-for="mall in mallsList" v-bind:key="mall.id" id="selectMallOption">
            {{ mall }}
          </option>
        </select>

        <input type="text" v-model="searchRestaurant" placeholder="Search Restaurant Name" id="filterRestaurant">

        <button type="button" id="filterSort" v-on:click="changeOrder()" title="Sort by ______">
...
export default {
  ...
  watch: {
    filterMall: function() {
      ...
      if (this.filterMall == "All") {
        ...
        query2 = database.collection('restaurants').where('mallName','==',this.filterMall)
      }
  ...
  methods: {
    ...
    filterQuery: function() {
      var query = database.collection('bookings').where('queueStatus','in',['waiting','current'])
    ...
    getQueueNumber: function(restaurantMall, paxGroup) {
      for (var i=0; i < this.currentQueue.length; i++) {
        if (this.currentQueue[i].restaurantMall == restaurantMall && this.currentQueue[i].paxGroup == paxGroup) {
    ...
    fetchData: function() {
      ...
      database.collection('bookings').where('queueStatus','==','current').get().then(snapshot => {
        snapshot.docs.forEach(doc => {
...
#dashboard > ul > li > table > tr > th,td {
  text-align: center;
  padding: 14px;
}
